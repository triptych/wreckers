...
- Every **5th wave** is a **Kraken** boss fight: it looms in from the side, and its aggression climbs while the beam is off it — let it go dark too long and its arm reaches the lighthouse for an instant game over. Keep the beam on it to hold it back. No ships or sharks spawn during the fight — but **kamikaze sharks** do: they ignore the Kraken entirely and beeline for the lighthouse itself, so holding the beam on the Kraken forever isn't a free win. Catch one in the beam to drive it off; let it land and it rams the light, spiking the Kraken's aggression while you weren't watching it. Every mermaid you've saved (lifetime, up to 5) swims out to shove the Kraken's aggression down, and the Light Wave halves it outright. Survive the clock and it flees.
...
- **The Kraken** — every 5th wave, `spawnBoss()`/`stepBoss()` take over `step()`: normal ship/shark/mermaid spawning pauses, and a single aggression value (0-100) drives the fight — it climbs while the beam is off the Kraken, falls while lit, and hitting 100 is an instant game over. The Kraken's stand-off distance is floored so it's always within beam range — otherwise calming it down would also drag it out of reach, stalling the fight. Lifetime-saved mermaids (capped at 5) spawn as allies that swim out and shove aggression down on contact. `spawnJawShark()`/the jaw-shark loop in `stepBoss()` periodically send a kamikaze shark straight at the lighthouse, ignoring the Kraken entirely — light it to drive it off, or it rams the lighthouse and spikes aggression, forcing the player off the Kraken instead of just parking the beam on it. Survive a wave-scaled timer and the Kraken flees.
